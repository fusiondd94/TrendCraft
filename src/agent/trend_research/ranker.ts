...
/** Coerce `Record<string, unknown>` to the serializable subset
 *  required by `TrendSource.rawMetadata` (string | number | boolean | null). */
function coerceRawMetadata(
  ...
  for (const [k, v] of Object.entries(input)) {
    if (
      ...
      // Stringify objects/arrays — fine for metadata the UI never
      // parses. A full recursive JSON-safe schema can replace this
      // if a source ever needs nested objects back as objects.
...
function buildTrend(
  ...
  const source: TrendSource = {
    ...
    // Coerce to the serializable subset — TanStack Start's typed
    // serializer rejects `Record<string, unknown>`. Sources pass
    // JSON-friendly values; anything else is stringified or dropped.
    rawMetadata: coerceRawMetadata(c.rawMetadata),
  };
