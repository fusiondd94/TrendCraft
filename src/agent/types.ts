...
export const TrendSourceSchema = z.object({
  ...
  // JSON-serializable only (no `unknown`) so the result can
  // cross TanStack Start's typed serializer. Sources must coerce
  // non-serializable values to strings before assigning here.
  rawMetadata: z
    .record(
      z.string(),
